{-# LANGUAGE DataKinds                  #-} -- 値コンストラクタを型に、型コンストラクタをKindに昇格
{-# LANGUAGE DefaultSignatures          #-} -- 型クラスのデフォルトのメソッド定義に特別な型を付けられる
{-# LANGUAGE ExistentialQuantification  #-} --
{-# LANGUAGE FlexibleContexts           #-}
{-# LANGUAGE FlexibleInstances          #-} -- 型変数を含まない独自定義型を型クラスのインスタンスにできる
{-# LANGUAGE GADTs                      #-} -- 多相データ型定義時に新たな構文を導入する
{-# LANGUAGE GeneralizedNewtypeDeriving #-} -- newtype定義のデータ型をderivingで各型クラスインスタンスにできる
{-# LANGUAGE KindSignatures             #-} -- 型変数に対してカインド注釈が付けられるようになる
{-# LANGUAGE MultiWayIf                 #-}
{-# LANGUAGE PolyKinds                  #-}
{-# LANGUAGE RankNTypes                 #-}
{-# LANGUAGE ScopedTypeVariables        #-} -- 型変数にもスコープの概念を持ち込む
{-# LANGUAGE TupleSections              #-}
{-# LANGUAGE TypeFamilies               #-} -- 型を返す関数を定義できる
{-# LANGUAGE TypeOperators              #-}

-- | Declarative options parser
-- | > https://github.com/tanakh/optparse-declarative/blob/master/src/Options/Declarative.hs
-- | 解説
-- | > https://blog.mudatobunka.org/entry/2016/12/11/140845
...
-- | Command line option
-- | TypeFamilies拡張により, 型クラス定義内部でtypeというキーワードが利用できる
-- | この場合, Value a は Value a 型「ではなく」、Value :: * -> * という型関数によって
-- | 導き出される何かしらの具体型(*)になる。
-- |
-- | 型関数 : Value
-- | 型関数のKind : * -> *
-- | 型関数にある型引数aを適用したもの : Value a
-- | 型関数にある型引数aを適用したときのKind : *
-- |
-- | つまり, Value a は例えば, Int型やMaybe String型などになる
-- | そのため, getの型も、何かしらの具体型aを受け取り、何かしらの具体型になる、と読むことができる。
-- | ...なかなかすげぇなこれ. ただこれ型付けがあってないようなものでは?
-- | 逆を返せば、型付けが義務付けられた世界の中で、型を自由に取れる関数定義ができるってことか?
